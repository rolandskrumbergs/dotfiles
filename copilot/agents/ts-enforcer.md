...
2. **Check against guidelines**: Does this follow project principles?
...
Find TypeScript files in the project, focusing on recently changed files. Exclude `node_modules`, `dist`, `build`.

#### 2. Check Compiler Configuration

Verify `tsconfig.json` has all strict mode flags enabled:
- `strict: true`
- `noImplicitAny: true`
...
- `any` types
- Type assertions (`as`)
- Ignore directives (`@ts-ignore`, `@ts-expect-error`)
- `interface` keyword for data structures
